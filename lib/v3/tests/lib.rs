...
use data_encoding_v3::DecodeKind::*;
use data_encoding_v3::{
    self as constants, self as data_encoding, DecodeError, DynEncoding, Specification,
};
...
macro_rules! test {
    (fn $t: ident; $($s: stmt);*;) => {
        #[test]
        fn $t() {
            fn test(b: &DynEncoding, x: &[u8], y: &[u8]) {
...
test! {
    fn hexlower;
    let b = constants::HEXLOWER.as_dyn();
    test(b, b"", b"");
    test(b, b"f", b"66");
    test(b, b"fo", b"666f");
...
test! {
    fn hexupper;
    let b = constants::HEXUPPER.as_dyn();
    test(b, b"", b"");
    test(b, b"f", b"66");
    test(b, b"fo", b"666F");
    test(b, b"foo", b"666F6F");
...
test! {
    fn base32hex;
    let b = constants::BASE32HEX.as_dyn();
    test(b, b"", b"");
    test(b, b"f", b"CO======");
    test(b, b"fo", b"CPNG====");
...
test! {
    fn base32_dnscurve;
    let b = constants::BASE32_DNSCURVE.as_dyn();
...
test! {
    fn base64url;
    let b = constants::BASE64URL.as_dyn();
    test(b, b"", b"");
    test(b, b"f", b"Zg==");
    test(b, b"fo", b"Zm8=");
    test(b, b"foo", b"Zm9v");
...
test! {
    fn base64_no_pad;
    let b = constants::BASE64_NOPAD.as_dyn();
    test(&b, b"", b"");
    test(&b, b"f", b"Zg");
    test(&b, b"fo", b"Zm8");
    test(&b, b"foo", b"Zm9v");
...
fn bit_order() {
    ...
    spec.bit_order = data_encoding_v3::BitOrder::LeastSignificantFirst;
...
fn ignore() {
    ...
    fn check(base: &DynEncoding, buf: &[u8], cmp: &[u8], shift: &[usize]) {
    ...
    fn forall(base: &DynEncoding, chars: &[u8], max: usize) {
...
#[test]
fn round_trip() {
    let test = |e: &DynEncoding| {
        assert_eq!(&e.specification().encoding().unwrap(), e);
    };
    test(data_encoding::HEXLOWER.as_dyn());
    test(data_encoding::HEXLOWER_PERMISSIVE.as_dyn());
    test(data_encoding::HEXUPPER.as_dyn());
    test(data_encoding::HEXUPPER_PERMISSIVE.as_dyn());
    test(data_encoding::BASE32.as_dyn());
    test(data_encoding::BASE32_NOPAD.as_dyn());
    test(data_encoding::BASE32HEX.as_dyn());
    test(data_encoding::BASE32HEX_NOPAD.as_dyn());
    test(data_encoding::BASE32_DNSSEC.as_dyn());
    test(data_encoding::BASE32_DNSCURVE.as_dyn());
    test(data_encoding::BASE64.as_dyn());
    test(data_encoding::BASE64_NOPAD.as_dyn());
    test(data_encoding::BASE64_MIME.as_dyn());
    test(data_encoding::BASE64_MIME_PERMISSIVE.as_dyn());
    test(data_encoding::BASE64URL.as_dyn());
    test(data_encoding::BASE64URL_NOPAD.as_dyn());
}
